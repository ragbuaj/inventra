...
> 13. **Next: Backend bank-FAM modules** — asset transfer/mutasi (`asset.asset_transfers`), stock opname (`stockopname.*`), or disposal GA/loss accounting (requires depreciation). Alternatively: extend field-permission `FilterView` enforcement to `requests`/`employees`/other modules (see §10 TODO).
