...
> diubah jadi identitas-dulu (Nama, Kode, Jenis, Induk, lalu Provinsi/Kota/Alamat) — *deviasi mockup
> `Master Data Kantor.dc.html` atas permintaan eksplisit user*. (3) **Menu "Impor Data" disembunyikan dari
> sidebar** (dihapus dari `appNav`; alur impor tetap lewat tombol Import per-layar master; halaman
> `/master/import` tetap ada). (4) **403 superadmin saat edit kantor** = footgun cache authz Redis basi
> (DB benar: superadmin punya `masterdata.office.manage` + scope `offices` global) — di-`FLUSHALL`;
> `*` tak bisa diberikan lewat UI RBAC (tak ada bug kode). Ingat: **`redis-cli FLUSHALL` wajib setelah
> seed/ubah authz via SQL** (lihat memori seed-flush-redis-authz). (5) **Kolom lantai (`floor_id`) pada
> departemen** — migrasi `000047` (nullable + index), sqlc, DTO/service (validasi *floor harus milik
> office departemen* → 400 `ErrFloorOfficeMismatch`), OpenAPI; frontend layar Referensi: field lantai
> **wajib + difilter per kantor** (USelect opsi lantai kantor terpilih, empty-state "No Data", office
> jadi wajib), resolusi nama office/floor di kolom tabel. *Field lantai adalah tambahan atas mockup
> Referensi — permintaan eksplisit user.* (6) **Mobile user-menu** (`UserMenu`): popover di-anchor ke
> `align:end` + collision-padding + lebar responsif (`max-w-[calc(100vw-16px)]`) agar tak melewati tepi
> layar sempit. **Gate:** backend build/vet/test (semua paket) + `go vet -tags=integration` hijau
