...
> 16. **Next: Wire Pegawai screen** (`/master/employees`) to `GET/POST/PUT/DELETE /api/v1/masterdata/employees` — scope `employees`; FK pickers for office/position/department; `useEmployees` is already a `useReference` consumer stubbed in #40. This is the next master-data sub-project.
