...
> 18. **Next session — pick the next real step.** All 23 mockup screens are built and every master-data + settings screen is wired to a real backend. Remaining frontend wiring needs new backend modules first. Candidates (see *Remaining* below): **(a)** build a backend feature module — **Assignment** (check-out/in) or **Maintenance** — then its screen; **(b)** wire **global search** backend (`/search`) + drop the last `mock/*` files; **(c)** start the **Bank-FAM** modules (transfer/mutasi, stock opname, disposal). Confirm priority before starting.
